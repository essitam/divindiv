...
function randomColor() {
  ...
  return 'rgb(' + r + ',' + g + ',' + b + ')';
    ...
    function N(f) {
        switch (f.keyCode) {
        case 40:
            A(-10);//this is the down key
            f.preventDefault();
            break;
        case 38:
            A(30);//this is the up key on the keyboard. 
            f.preventDefault();
            break;
        case 37:
            B(-1);
        ...
        for (n = j.length; h < n; h++) {
            ...
            for (o = a.nodes.length; d < o; d++) {
                ...
                if (f.debug) {
                    ...
                    l.quadraticCurveTo(c.position.x, c.position.y, c.position.x + (p.position.x - c.position.x) / 2, c.position.y + (p.position.y - c.position.y) / 2)
            }
            l.stroke();
            l.fill()
        }
    ...
    var g = {
        ...
        C = [{
			...
			{
            fillStyle: "rgb(255,110,30)"},
			 
			{
			  fillStyle: "rgb(255,231,69)"},  
			
			 {
			  fillStyle: "rgb(255,117,69)"},
			 {
			  fillStyle: "rgb(95,69,255)"},  
			
			 {
			  fillStyle: "rgb(116,213,97)"},  
...
Region.prototype.inflate = function(b, e) {
    ...
    this.top = Math.min(this.top, e); //this.top = Math.min(this.top+100, e); >>accidentally traces image/ also b + 100, creates interesting image traces of where the blob used to be. 
...
Blob.prototype.split = function() {
    ...
    for (s = 0; s < this.nodes.length; s++)
        e += this.nodes[s].position.x;
    for (s = 0; s < k.nodes.length; s++)
        z += k.nodes[s].position.x;
...
Blob.prototype.merge = function(b) {
    ...
    for (this.velocity.y += b.velocity.y * 0.5; b.nodes.length;)
