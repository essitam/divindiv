...
//>> if i get rid of these things, nothing will be drawn on a simple mousedown. BUT if i keep it, it gives me an opportunity to draw something different. 
 ...
 //ctx.fillStyle = randomColor();
	ctx.fillRect(mouseX, mouseY + (340 * Math.random()), 1 + (50* Math.random()), 1 + (70* Math.random()));  
  ctx.fillStyle = randomColor();
	
	/*ctx2.fillRect(mouseX+(700* Math.random()), mouseY + (340 * Math.random()), 10 + (70* Math.random()), 10 + (70* Math.random()));  
  ctx2.fillStyle = randomColor();*/
	//these are the big squares.... 
}
...
function tool(event) {
  ...
  if (current == "draw") {
    /*ctx.fillStyle = randomColor();
    ctx.fillRect(mouseX, 10, 5, size*20);*/
	  //this is the tiny origin pixel, currently at the top, where the long thing colors draw from.
	  
	  //this is how i changed the cube into a rectangle.
